    ...
    public class AppThemeDataPrivateList
    {
        ...
        public AppThemeDataPrivateList(string selectedsystemkey, bool useCache)
        {
            try
            {
                ...
                if (useCache)
                {
                    List = (List<SimplisityRecord>)CacheUtils.GetCache(cachekey, "apptheme");
                    var cacheRtn = CacheUtils.GetCache(cachekey + "ERROR", "apptheme");
                    if (cacheRtn == null)
                    {
                        Error = false;
                    }
            ...
            if (SelectedSystemKey != "")
            {
                ...
                if (ftpConnect.IsValid)
                {
                    ...
                    foreach (SimplisityRecord a in SortedList)
                    {
                        ...
                        var onlineIndex = (OnlineAppThemeIndex)CacheUtils.GetCache(appTheme.SystemKey + "_privateIndex");
                        if (onlineIndex == null)
                        {
                            onlineIndex = new OnlineAppThemeIndex(appTheme.SystemKey,"private");
                ...
                CacheUtils.SetCache(cachekey + "ERROR", Error, "apptheme");
            }
        }
