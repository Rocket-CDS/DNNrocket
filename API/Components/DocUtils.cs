...
namespace DNNrocketAPI.Components
{
    public class DocUtils
    {
        ...
        public static Dictionary<string, string> UploadSecureBase64file(string[] filenameList, string[] filebase64List, string docFolderMapPath)
        {
            var rtn = new Dictionary<string, string>();
            if (filebase64List.Length == filenameList.Length && filenameList.Length > 0)
            {
                ...
                foreach (var ncode in filenameList)
                {
                    ...
                    if (!ImgUtils.IsImageFile(Path.GetExtension(fname)) && IsSafeFileType(Path.GetExtension(fname)))
                    {
                        ...
                        rtn.Add(docFileMapPath, docFriendlyName);
                        lp += 1;
                    }
                    else
                    {
        ...
        public static Dictionary<string, string> UploadBase64fileDict(string[] filenameList, string[] filebase64List, string docFolderMapPath)
        {
            var rtn = new Dictionary<string, string>();
            if (filebase64List.Length == filenameList.Length && filenameList.Length > 0)
            {
                ...
                foreach (var ncode in filenameList)
                {
                    ...
                    if (!ImgUtils.IsImageFile(Path.GetExtension(fname)) && IsSafeFileType(Path.GetExtension(fname)))
                    {
                        ...
                        rtn.Add(docFileMapPath, Path.GetFileName(docFileMapPath));
                        lp += 1;
                    }
                    else
                    {
